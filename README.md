...
- Battery %, range (km/mi), charging status and time-to-full, refreshed every 5 minutes
- Choose what the menu bar shows
- Password stored in the macOS Keychain — never in plaintext
...
and are not notarized, so on first launch: **right-click → Open → Open**.
